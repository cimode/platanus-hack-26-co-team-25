...
const people: Person[] = [
  ...
  },
  {
    ...
    name: "Mateo",
    latents: {
      regulation: { mean: 0.66 },
      politeness: { mean: 0.71 },
      reliability: { mean: 0.62 },
      agency: { mean: 0.45 },
    },
    declared: {
      distanceBand: 0,
  ...
  },
  {
    ...
    name: "Lucas",
    latents: {
      regulation: { mean: 0.22, se: 0.45 },
      politeness: { mean: 0.58 },
      reliability: { mean: 0.55 },
      agency: { mean: 0.4 },
    },
    declared: {
      distanceBand: 2,
  ...
  },
  {
    ...
    latents: {
      regulation: { mean: 0.6 },
      politeness: { mean: 0.55 },
      reliability: { mean: 0.8 },
      agency: { mean: 0.85, se: 0.3 },
    },
    declared: {
      distanceBand: 1,
      lifeShape: {
  ...
  },
  {
    ...
    latents: {
      regulation: { mean: 0.7 },
      politeness: { mean: 0.74 },
      reliability: { mean: 0.68 },
      agency: { mean: 0.35 },
    },
    declared: {
      distanceBand: 0,
  ...
  },
  {
    ...
    name: "Carla",
    latents: {
      regulation: { mean: 0.55 },
      politeness: { mean: 0.62 },
      reliability: { mean: 0.58 },
      agency: { mean: 0.5 },
    },
    declared: {
      distanceBand: 0,
...
function printRoom(lens: Lens, opts?: { agencyOverlay?: boolean }): void {
  ...
  for (const e of ranked) {
    const flags = Object.entries(e.flags)
      .map(([k, v]) => `${k}=${f(v as number)}`)
      .join(" ");
    const drivers = e.drivers
      .map((d) => `${d.term}:${f(d.contribution)}`)
      .join(" ");
    console.log(
  ...
  const bySim = [...ranked].sort(
    (x, y) => y.sim - x.sim || (x.id < y.id ? -1 : 1)
  );
  console.log(
    `  w_sim order: ${bySim.map((e) => `${e.id}(${f(e.sim)})`).join(" > ") || "(none)"}`
  );
  ...
  const excluded = excludedFromRoom(people, SUBJECT, lens, opts).filter(
    (x) => !x.reason.startsWith("suppressed")
  );
  for (const x of excluded)
    console.log(`  excluded ${x.id.padEnd(7)} — ${x.reason}`);
}

console.log(
  `ROOM: ${people.length} people | subject: ${SUBJECT} | deterministic run`
);
...
console.log(
  `sofia x mateo   (romantic): eligible=${kidsGate.eligible} reason="${kidsGate.reason}"`
);
const pw = scorePair(people[0], people[2], "romantic"); // sofia x lucas
console.log(
  `sofia x lucas   (romantic): pursueWithdraw=${f(pw.flags.pursueWithdraw ?? 0)} sim=${f(pw.sim)} (includes fixed w_sim penalty)`
);
const cofounders = scorePair(people[0], people[3], "business", {
  agencyOverlay: true,
}); // sofia x diego
console.log(
  `sofia x diego   (business+overlay): bothHighAgency=${f(cofounders.flags.bothHighAgency ?? 0)} rank=${f(cofounders.rank)}`
);
const degraded = scorePair(people[0], people[4], "friendship"); // sofia x nina
console.log(
  `sofia x nina    (friendship, nina has zero latents → prior 0.5/se 0.6): rank=${f(degraded.rank)} band=${degraded.band}`
);
...
console.log(
  `suppressed pair (romantic): carries no data — eligible=${suppressed.eligible} rank=${f(suppressed.rank)} drivers=${suppressed.drivers.length} flags=${Object.keys(suppressed.flags).length}`
);
