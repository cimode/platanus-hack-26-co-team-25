...
describe("our copy of EventKind has not drifted from the engine", () => {
   ...
   */
  const SHARED_TS = new URL("../../../../timeline/shared.ts", import.meta.url);
  ...
  function upstreamEventKinds(): string[] {
    ...
    if (declaration === null) {
      throw new Error(
        `No 'export type EventKind = ...;' found in ${SHARED_TS.pathname}. ` +
          "The union was renamed or reshaped upstream; reconcile " +
          "src/lib/domain/reveal/timeline.ts with it."
      );
    }
