...
import type { Lens, PairScore, Person, TermName } from "../matching/engine.ts";
...
/**
 * The narration seam, INVERTED (issue #33).
 *
 * In the bake-off harness `approach-b/index.ts` imported `narrate` / `nominate`
 * from `lib/narrator.ts`, which is the one module that touches an SDK. Here
 * they arrive as a PARAMETER instead, so nothing under `src/lib/domain/**`
 * imports `ai`, `zod` or a gateway client -- the rule `docs/testing.md` states
 * as "if a module imports an SDK, it is not an engine module".
 *
 * That is also what makes the structure testable with no model at all: the
 * deterministic `mockNarrator` is a `TimelineNarrator`, and so is the adapter
 * over `LlmPort`.
 */
export interface TimelineNarrator {
  /** One sentence per beat, in beat order. Never throws: a failed beat takes
   *  its deterministic fallback text and the timeline still ships. */
  narrate(
    beats: readonly Beat[],
  ...
  /** Proposes ONE bonus arc from the grammar. The CALLER verifies the claim. */
  nominate(
    a: Person,
    b: Person,
    score: PairScore,
...
export interface NarrateResult {
  ...
  /** Live parallel only: beats whose own call failed and fell back to mock prose. */
  mockFallbacks?: number;
}

/** The fixed vocabulary a nomination must choose from. */
...
/**
 * `narrator` is OPTIONAL and defaults to the deterministic `mockNarrator`, so
 * every structural test runs offline and the whole timeline degrades to
 * deterministic prose rather than failing when no model is reachable. The
 * adapter passes the live one; nothing in this package can reach a model on
 * its own.
 */
...
export function stateAt(
  byYear: Map<number, ThreadedState>,
  year: number
): ThreadedState {
...
export function hazardShape(year: number): number {
  ...
  return (
    HAZARD_PEAK_LEVEL *
    Math.exp(-HAZARD_DECLINE_RATE * (year - HAZARD_PEAK_END_YEAR))
  );
}
...
export function sampleDissolutionYear(
  ...
  for (let y = 2; y <= horizonYears; y++) {
    // year 1 is formation — never dissolve there
    const p =
      HAZARD_BASE_P * hazardShape(y) * (1 - HAZARD_SIM_DAMPING * simNorm);
    if (rng() < p) return y;
  }
  return null;
}
...
const BANNED_PATTERNS: ReadonlyArray<{ category: string; re: RegExp }> = [
  {
    category: "gottman",
    re: /\b(criticism|contempt|defensiveness|stonewall\w*)\b/gi,
  },
  { category: "ultimatum", re: /\bultimatum\w*\b/gi },
  {
    category: "moral-attribution",
    re: /\b(selfish|toxic|narcissis\w*|abus\w*|manipulat\w*|lazy|liar|lying|cruel|pathetic|worthless|deadbeat)\b/gi,
  },
  {
    category: "infidelity",
    re: /\b(cheat\w*|affair|infidelity|unfaithful|betray\w*|mistress)\b/gi,
  },
  {
    category: "illness-death",
    re: /\b(cancer|tumou?r|terminal|diagnos\w*|illness|disease|hospitali[sz]\w*|death|dies|died|dying|funeral|suicid\w*|grief|widow\w*)\b/gi,
  },
  {
    category: "substances",
    re: /\b(drugs?|alcohol\w*|drunk|wasted|hangover|addict\w*|cocaine|weed|vap(e|ing)|cigarette\w*|smoking|sober|rehab)\b/gi,
  },
  {
    category: "religion",
    re: /\b(religio\w*|church|mosque|synagogue|temple|god|allah|jesus|pray\w*|bible|quran|atheis\w*|baptis\w*)\b/gi,
  },
  {
    category: "politics",
    re: /\b(politic\w*|election\w*|president\w*|congress|senat\w*|left-wing|right-wing|liberal\w*|conservativ\w*|protest\w*)\b/gi,
  },
  {
    category: "money-shame",
    re: /\b(bankrupt\w*|broke|penniless|foreclos\w*|evict\w*|cheapskate|stingy|gold-?digg\w*|freeload\w*)\b/gi,
  },
];
...
export function kidEventAllowed(
  ...
  if (!a.gates.romantic?.wantsKids || !b.gates.romantic?.wantsKids)
    return false;
...
export function isDegradedPair(a: Person, b: Person): boolean {
  return LATENT_NAMES.some(
    (n) => a.latents[n] === undefined || b.latents[n] === undefined
  );
}
...
export function checkSchema(t: Timeline): ValidationIssue[] {
  ...
  if (!t.personA?.id || !t.personB?.id)
    out.push(issue("schema.persons", "error", "personA/personB refs missing"));
  if (!Array.isArray(t.arcs) || t.arcs.length === 0)
    out.push(issue("schema.arcs", "error", "no arcs"));
  if (!Array.isArray(t.events)) {
    out.push(issue("schema.events", "error", "events is not an array"));
    return out;
  }
  if (t.events.length < c.minEvents)
    out.push(
      issue(
        "schema.min-events",
        "error",
        `only ${t.events.length} events; minimum is ${c.minEvents} (degraded mode included)`
      )
    );
  if (t.events.length > c.maxEvents)
    out.push(
      issue(
        "schema.max-events",
        "warn",
        `${t.events.length} events exceeds the ${t.lens} cap of ${c.maxEvents}`
      )
    );
  if (t.meta?.narration !== "live" && t.meta?.narration !== "mock")
    out.push(
      issue("schema.meta", "error", 'meta.narration must be "live" | "mock"')
    );
  ...
  if (t.lens === "friendship") {
    ...
    for (const forbidden of ["dissolution", "horizonYears", "epilogue"]) {
      if (forbidden in anyT)
        out.push(
          issue(
            "schema.friendship-structural",
            "error",
            `friendship timeline must not carry "${forbidden}" (PILLARS §6.1: no survival curve, no duration claim)`
          )
        );
    }
  } else {
    ...
    if (
      typeof t.horizonYears !== "number" ||
      t.horizonYears < span[0] ||
      t.horizonYears > span[1]
    ) {
      out.push(
        issue(
          "schema.horizon",
          "error",
          `horizonYears must be in [${span[0]}, ${span[1]}]`
        )
      );
    }
    if (t.dissolution !== null) {
      if (t.dissolution.year < 2 || t.dissolution.year > t.horizonYears)
        out.push(
          issue(
            "schema.dissolution-year",
            "error",
            "dissolution year outside (1, horizon]"
          )
        );
      if (!t.arcs.some((arc) => arc.id === t.dissolution?.arcId))
        out.push(
          issue(
            "schema.dissolution-arc",
            "error",
            "dissolution.arcId references no arc"
          )
        );
    }
  }
  ...
  for (const e of t.events) {
    if (!arcIds.has(e.arcId))
      out.push(
        issue(
          "schema.event-arc",
          "error",
          `event year ${e.year} references unknown arc "${e.arcId}"`
        )
      );
    if (!c.allowedKinds.includes(e.kind))
      out.push(
        issue(
          "schema.event-kind",
          "error",
          `kind "${e.kind}" not allowed in the ${t.lens} lens`
        )
      );
    if (!Number.isInteger(e.year) || e.year < 1)
      out.push(
        issue(
          "schema.event-year",
          "error",
          `event year ${e.year} must be a positive integer`
        )
      );
    if (!e.text || e.text.trim().length === 0)
      out.push(
        issue(
          "schema.event-text",
          "error",
          `event year ${e.year} has empty text`
        )
      );
  }
    ...
    if (t.events[i].year < t.events[i - 1].year) {
      out.push(
        issue(
          "schema.event-order",
          "error",
          "events not sorted by year ascending"
        )
      );
      break;
    }
...
export function checkFrictionArc(
  t: Timeline,
  score: PairScore
): ValidationIssue[] {
  const out: ValidationIssue[] = [];
  ...
  if (frictionArcs.length === 0) {
    out.push(
      issue(
        "friction.missing",
        "error",
        "no friction arc — every timeline must carry at least one (RESEARCH §4.3: the honesty feature)"
      )
    );
    return out;
  }
  ...
  if (!t.events.some((e) => ids.has(e.arcId))) {
    out.push(
      issue(
        "friction.no-events",
        "error",
        "friction arc exists but produced no events"
      )
    );
  }
  if (
    score.friction &&
    !frictionArcs.some((a) => a.sourceTerm === score.friction?.term)
  ) {
    out.push(
      issue(
        "friction.term-mismatch",
        "warn",
        `friction arc does not cite the scored friction term "${score.friction.term}"`
      )
    );
  }
  return out;
...
export function checkKidGates(
  ...
  const dissolutionYear =
    t.lens === "friendship" ? null : (t.dissolution?.year ?? null);
  for (const e of t.events) {
    ...
    if (!kidEventAllowed(a, b, t.lens, opts, alive)) {
      out.push(
        issue(
          "kids.gate",
          "error",
          `kid event at year ${e.year} fails the kid gate (wantsKids both + offspring consent both + relationship alive that year — AUDIT S11)`
        )
      );
    }
  }
...
export function checkCoherence(t: Timeline): ValidationIssue[] {
  ...
  if (t.lens === "friendship") {
    if (
      t.events.some((e) => e.kind === "dissolution" || e.kind === "epilogue")
    ) {
      out.push(
        issue(
          "coherence.friendship-ending",
          "error",
          "friendship vignettes must not contain dissolution/epilogue events"
        )
      );
    }
    return out;
  ...
  if (dYear === null && dissolutionEvents.length > 0)
    out.push(
      issue(
        "coherence.dissolution-header",
        "error",
        "dissolution event present but timeline header says together at horizon"
      )
    );
  if (dYear !== null) {
    if (dissolutionEvents.length !== 1)
      out.push(
        issue(
          "coherence.dissolution-count",
          "error",
          `expected exactly 1 dissolution event, found ${dissolutionEvents.length}`
        )
      );
    else if (dissolutionEvents[0].year !== dYear)
      out.push(
        issue(
          "coherence.dissolution-year",
          "error",
          "dissolution event year does not match timeline.dissolution.year"
        )
      );
    const after = t.events.filter((e) => e.year > dYear);
    const nonEpilogue = after.filter((e) => e.kind !== "epilogue");
    if (nonEpilogue.length > 0)
      out.push(
        issue(
          "coherence.after-dissolution",
          "error",
          `${nonEpilogue.length} non-epilogue event(s) after dissolution year ${dYear}`
        )
      );
    if (after.filter((e) => e.kind === "epilogue").length > 1)
      out.push(
        issue(
          "coherence.epilogue-count",
          "error",
          "at most one epilogue event after dissolution"
        )
      );
  } else if (t.events.some((e) => e.kind === "epilogue")) {
    out.push(
      issue(
        "coherence.epilogue-alive",
        "error",
        "epilogue event without a dissolution"
      )
    );
  }
  ...
  for (const arc of t.arcs) {
    for (const beat of arc.beats) {
      if (beat.kind === "kid" && !beat.delta?.addKid)
        out.push(
          issue(
            "coherence.kid-delta",
            "warn",
            `kid beat at year ${beat.year} carries no addKid delta`
          )
        );
      if (beat.kind === "pet" && !beat.delta?.addPet)
        out.push(
          issue(
            "coherence.pet-delta",
            "warn",
            `pet beat at year ${beat.year} carries no addPet delta`
          )
        );
      if (beat.kind === "move" && !beat.delta?.location)
        out.push(
          issue(
            "coherence.move-delta",
            "warn",
            `move beat at year ${beat.year} carries no location delta`
          )
        );
    }
  }
...
export function checkSafety(t: Timeline): ValidationIssue[] {
  const out: ValidationIssue[] = [];
  const texts: Array<[string, string]> = t.events.map((e) => [
    `event y${e.year}`,
    e.text,
  ]);
  for (const arc of t.arcs) texts.push([`arc ${arc.id} label`, arc.label]);
  if (t.lens !== "friendship" && t.epilogue)
    texts.push(["epilogue", t.epilogue]);
  for (const [where, text] of texts) {
    for (const hit of scanBanned(text)) {
      out.push(
        issue(
          "safety.banned",
          "error",
          `${where}: banned ${hit.category} term "${hit.match}"`
        )
      );
    }
    for (const m of scanSurvivalClaims(text)) {
      out.push(
        issue(
          "safety.survival-claim",
          "error",
          `${where}: numeric survival/probability claim "${m}" (AUDIT S10)`
        )
      );
    }
  }
