...
import {
  DOMAIN_LABELS,
  FRICTION_DOMAINS,
  frictionOutcomeWeights,
  frictionPatternWeights,
  grammarSpace,
  outcomeWeights,
  PATTERNS,
  warmDomainWeights,
...
const clampYear = (y: number, last: number): number =>
  Math.max(1, Math.min(last, y));
...
export const generateTimeline: GenerateTimeline = async (
  a,
  b,
  score,
  lens,
  ...
  const { narrate, nominate } = narrator;
  ...
  const frictionTerm: TermName =
    score.friction?.term ??
    (lens === "business" ? "reliability" : "commonGround");
  ...
  const dYear =
    lens === "friendship"
      ? null
      : sampleDissolutionYear(rng, score.sim, horizon);
  ...
  if (lens === "friendship") {
    ...
    fBeats.push({
      year: clampYear(clashY + 1, lastYear),
      step: isStress ? "proof" : "repair",
    });
    if (fOutcome === "strengthens")
      fBeats.push({ year: clampYear(clashY + 2, lastYear), step: "settle" });
  }
  const frictionArc: ArcPlan = {
    id: "arc-friction",
    role: "friction",
    sourceTerm: frictionTerm,
    label: `Where it grinds — ${TERM_LABELS[frictionTerm]}`,
    pattern: fPattern,
    domain: frictionDomain,
    outcome: fOutcome,
    beats: fBeats,
    hintTerm: frictionTerm,
  };

  // -- 3. warm driver arcs (score-conditioned pattern × domain × outcome) ---
  const warmExcluded = new Set<string>([frictionDomain]);
  if (lens === "romantic") {
    warmExcluded.add("kids");
    warmExcluded.add("conflict-recovery");
  }
  if (lens === "business") {
    warmExcluded.add("exit");
    warmExcluded.add("decision-rights");
  }
  ...
  for (let i = 0; i < 2; i++) {
    ...
    if (lens === "business" && i === 0 && !warmExcluded.has("product")) {
      ...
      if (pattern !== "spark" && pattern !== "slow-build")
        pattern = "slow-build";
    } else {
  ...
  if (lens === "romantic" && kidEventAllowed(a, b, lens, opts, true)) {
    ...
    const maxKid = Math.max(1, window);
    {
      ...
      extras.push({
        id: "arc-kid",
        role: "texture",
        sourceTerm: "eligibility",
        label: DOMAIN_LABELS.kids,
        pattern: "leap",
        domain: "kids",
        outcome: "strengthens",
  ...
  if (
    ...
    if (domain !== frictionDomain) {
      ...
      extras.push({
        ...
        role: "flag",
        sourceTerm: "agency",
        label: "Two hands on the wheel",
        pattern: "grind-repair",
        domain,
        outcome: "strengthens",
        beats: [
          { year: y, step: "clash" },
          { year: clampYear(y + 1, lastYear), step: "repair" },
        ],
        flag: "bothHighAgency",
      });
    }
  ...
  if (
    lens === "romantic" &&
    score.flags.pursueWithdraw !== undefined &&
    frictionDomain !== "conflict-recovery"
  ) {
  ...
  const avgExit =
    lens === "business" && a.gates.business && b.gates.business
  ...
  if (
    lens === "friendship" &&
    frictionTerm !== "distance" &&
    frictionTerm !== "regulation"
  ) {
    const isDriver = score.drivers.some((d) => d.term === "distance");
    const y = randInt(rng, 2, Math.max(2, horizon - 1));
    extras.push({
      id: "arc-distance",
      role: isDriver ? "driver" : "texture",
      sourceTerm: "distance",
      label: DOMAIN_LABELS["distance-texture"],
      pattern: "grind-repair",
      domain: "distance-texture",
      outcome: "strengthens",
      beats: [
        { year: y, step: "clash" },
        { year: clampYear(y + 1, horizon), step: "repair" },
      ],
      hintTerm: "distance",
    });
  }
  ...
  const quietArc: ArcPlan | null =
    lens !== "friendship" &&
    dYear === null &&
    horizon >= (lens === "business" ? 9 : 10)
      ? {
          id: "arc-quiet",
          role: "texture",
          sourceTerm: null,
          ...
          pattern: "spark",
          domain: lens === "business" ? "work-rhythm" : "ritual",
          outcome: "strengthens",
  ...
  if (verdict.admitted) {
    ...
    bonusArc = {
      id: "arc-bonus",
      role: "bonus",
      sourceTerm: verdict.sourceTerm,
      label: `Off the map — ${DOMAIN_LABELS[nomRes.nomination.domain] ?? nomRes.nomination.domain}`,
      pattern,
      domain: nomRes.nomination.domain,
      outcome: nomRes.nomination.outcome as OutcomeName,
      beats: steps.map((step, j) => ({
        year: clampYear(y0 + j, lastYear),
        step,
      })),
      hintTerm:
        verdict.claimKind === "friction"
          ? (verdict.sourceTerm ?? undefined)
          : undefined,
    };
  }

  // -- 6. event budget by priority ------------------------------------------
  const priorityOrder: Array<ArcPlan | null> =
    lens === "friendship"
  ...
  const realizeRng = mulberry32(
    hashSeed(opts.seed, "approach-b/realize", a.id, b.id, lens)
  );
    ...
    const r = bp.special
      ...
      : arc.flag
        ? realizeFlag(
            ctx,
            arc.flag,
            bp.step === "clash" || bp.step === "pressure" ? "clash" : "repair"
          )
        : arc.hintTerm &&
            (bp.step === "clash" ||
              bp.step === "pressure" ||
  ...
  try {
    ...
    /* a broken renderer is not a generation failure */
  }
  ...
  const meta: TimelineMeta = {
    ...
    ...(narrated.petGuardReplacements !== undefined
      ? { petGuardReplacements: narrated.petGuardReplacements }
      : {}),
