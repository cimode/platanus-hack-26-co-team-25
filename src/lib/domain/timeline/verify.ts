...
export function verifyTriggerClaim(
  ...
  if (!space.patterns.includes(nom.pattern))
    return reject(`pattern "${nom.pattern}" outside the grammar`);
  if (!space.domains.includes(nom.domain))
    return reject(`domain "${nom.domain}" outside the ${lens} grammar`);
  if (!space.outcomes.includes(nom.outcome))
    return reject(`outcome "${nom.outcome}" outside the grammar`);
  ...
  const m = /^(driver|friction|flag):([A-Za-z]+)$/.exec(
    nom.triggerClaim.trim()
  );
  ...
  if (kind === "driver") {
    if (!isTermName(name))
      return reject(`unknown term "${name}" in driver claim`);
    const hit = score.drivers.find(
      (d) => d.term === name && d.contribution > 0
    );
    if (!hit) return reject(`"${name}" is not one of this pair's top drivers`);
    return {
      admitted: true,
      reason: `driver:${name} verified (contribution ${hit.contribution.toFixed(3)})`,
      claimKind: kind,
      sourceTerm: name,
    };
  }
  ...
  if (kind === "friction") {
    if (!isTermName(name))
      return reject(`unknown term "${name}" in friction claim`);
    if (score.friction?.term !== name) {
      return reject(
        `"${name}" is not this pair's friction term (actual: ${score.friction?.term ?? "none"})`
      );
    }
    return {
      admitted: true,
      reason: `friction:${name} verified (shortfall ${score.friction.shortfall.toFixed(3)})`,
      claimKind: kind,
      sourceTerm: name,
    };
  }
  ...
  if (name === "bothHighAgency") {
    ...
    return {
      admitted: true,
      reason: `flag:bothHighAgency verified (p=${score.flags.bothHighAgency.toFixed(2)})`,
      claimKind: kind,
      sourceTerm: "agency",
    };
  }
  if (name === "pursueWithdraw") {
    ...
    return {
      admitted: true,
      reason: "flag:pursueWithdraw verified",
      claimKind: kind,
      sourceTerm: "distance",
    };
  }
