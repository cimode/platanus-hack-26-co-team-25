...
import type { GrammarSpace } from '../lib/narrator.ts';

// ---------------------------------------------------------------------------
// Patterns × outcomes
// ---------------------------------------------------------------------------

export type PatternName = 'spark' | 'slow-build' | 'grind-repair' | 'leap' | 'ritual' | 'stress-test';
export type OutcomeName = 'strengthens' | 'lingers' | 'redirects';
export type StepName =
  | 'begin' | 'settle'             // spark: something starts, then becomes how they operate
  | 'seed' | 'progress' | 'payoff' // slow-build: intent → practice → payoff
  | 'clash' | 'repair'             // grind-repair: friction surfaces, then a repair
  | 'decide' | 'aftermath'         // leap: one decisive call, then its settling
  | 'start' | 'defend'             // ritual: a tradition starts, then survives a hard year
  | 'pressure' | 'proof';          // stress-test: outside pressure, then the hold
...
export const PATTERN_NAMES = Object.keys(PATTERNS) as unknown as readonly PatternName[];
export const OUTCOMES: readonly OutcomeName[] = ['strengthens', 'lingers', 'redirects'];

/** Patterns the friction pillar boosts (grind is the default honest shape). */
export const FRICTION_PATTERNS: readonly PatternName[] = ['grind-repair', 'stress-test'];
/** Patterns warm (driver/texture) arcs sample from. */
export const WARM_PATTERNS: readonly PatternName[] = ['spark', 'slow-build', 'ritual', 'leap'];
 ...
 */
export const DRIVER_DOMAINS: Record<Lens, Partial<Record<TermName, readonly string[]>>> = {
  romantic: {
...
export const FRICTION_GAP_NOUN: Record<string, string> = {
  'work-balance': 'hours', 'conflict-recovery': 'recovery-rhythm', home: 'day-to-day',
  ritual: 'shared-time', relocation: 'geography', travel: 'pace', pets: 'care',
  kids: 'timing', craft: 'practice',
  'work-rhythm': 'rhythm', runway: 'follow-through', 'decision-rights': 'decision',
  product: 'product-vision', hiring: 'team', pivot: 'direction', exit: 'timing',
  'first-client': 'traction',
  reunion: 'calendar', hobby: 'overlap', food: 'taste', media: 'canon',
  project: 'momentum', 'distance-texture': 'silence', trip: 'pace',
};

/** Human labels for warm/bonus arcs, per domain (user-facing, safety-scanned). */
export const DOMAIN_LABELS: Record<string, string> = {
  home: 'Making home', relocation: 'The city question', travel: 'Out the door',
  pets: 'The small roommate', kids: 'The kid year', ritual: 'Standing ritual',
  craft: 'The shared craft', 'work-balance': 'Hours and us',
  'conflict-recovery': 'The way back',
  runway: 'Runway math', 'first-client': 'First yes', 'decision-rights': 'Decision rights',
  hiring: 'First hire', product: 'The build', pivot: 'The turn', exit: 'The horizon',
  'work-rhythm': 'Operating rhythm',
  trip: 'The trip', hobby: 'The obsession', food: 'The rotation', media: 'The canon',
  project: 'The side project', reunion: 'The gathering problem',
...
export function grammarSpace(lens: Lens): GrammarSpace {
  return {
    ...
    domains: LENS_CONSTRAINTS[lens].domains.filter((d) => !NOMINATION_EXCLUDED[lens].includes(d)),
    outcomes: OUTCOMES,
  };
}
...
export function weightedPick<T>(rng: () => number, entries: ReadonlyArray<readonly [T, number]>): T {
  let total = 0;
...
export function warmPatternWeights(score: PairScore, sharedCount: number): Array<[PatternName, number]> {
...
export function frictionPatternWeights(score: PairScore): Array<[PatternName, number]> {
...
export function frictionOutcomeWeights(score: PairScore): Array<[OutcomeName, number]> {
  const q = simNorm(score);
  return [
    ['lingers', 1 + (1 - q)],
...
export function warmDomainWeights(
  ...
  for (const d of LENS_CONSTRAINTS[lens].domains) {
    ...
    for (const dr of score.drivers) {
      ...
      if (doms && doms.includes(d)) w += Math.max(0, dr.contribution) * 8;
    }
    out.push([d, w]);
  }
  return out;
