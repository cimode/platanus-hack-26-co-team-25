/**
 * engine.test.ts — property tests for matching/engine.ts.
 *
 * Zero runtime dependencies, plain assertions, erasable-types-only syntax.
 * Run: node --experimental-strip-types matching/engine.test.ts
 *
...
interface MkOpts {
  ...
  team?: string; track?: string; cohort?: number; acq?: string[];
  gender?: Gender; interestedIn?: Gender[]; single?: boolean; ageBand?: number; wantsKids?: boolean;
  noRomGate?: boolean; noBizGate?: boolean;
  risk?: number; exit?: number; redlines?: boolean;
  consentRom?: boolean; consentBiz?: boolean; consentFri?: boolean;
  photo?: boolean;
}
...
function mk(id: string, o: MkOpts = {}): Person {
  ...
  return {
    ...
    declared: {
      distanceBand: o.distanceBand === 'missing' ? undefined : (o.distanceBand ?? 1),
      lifeShape: {
    ...
    structural: { team: o.team, track: o.track ?? 'sim', cohort: o.cohort ?? 2, acquaintances: o.acq ?? [] },
    gates: {
      romantic: o.noRomGate ? undefined : {
      ...
      business: o.noBizGate ? undefined : {
        riskPosture: o.risk ?? 1,
        exitHorizon: o.exit ?? 1,
        redlinesOk: o.redlines ?? true,
      },
    },
    consent: { romantic: o.consentRom ?? true, business: o.consentBiz ?? true, friendship: o.consentFri ?? true },
...
test('P1b published PILLARS §3 cells exact (business under agencyOverlay)', () => {
  const PUB: Record<Lens, { rank: Record<TermName, number>; sim: Record<TermName, number> }> = {
    romantic: {
  ...
  for (const lens of LENSES) {
    ...
    for (const which of ['rank', 'sim'] as const) {
      for (const t of TERMS) {
        assert(w[which][t] === PUB[lens][which][t],
...
test('P2 symmetry: scorePair(a,b) ≈ scorePair(b,a) — rank/sim/eligible/band/flags', () => {
  const people: Person[] = [
    ...
    mk('s2', { team: 't1', gender: 'M', agc: 0.9, reg: 0.2, distanceBand: 3, tags: ['music', 'gaming'], cohort: 3 }),
    mk('s3', { noLatents: true, distanceBand: 'missing', gender: 'NB', tags: [], cohort: 5, track: 'ai' }),
    mk('s4', { gender: 'M', pol: 0.9, rel: 0.9, cap: 0, chrono: 3, wantsKids: false, tags: ['food', 'art'], acq: ['s1'] }),
    mk('s5', { gender: 'F', agc: 0.85, cap: 3, risk: 2, exit: 0, tags: ['hiking', 'food', 'music', 'art'] }),
    ...
    for (let i = 0; i < people.length; i++) {
      for (let j = i + 1; j < people.length; j++) {
        for (const lens of LENSES) {
          ...
          assert(ab.eligible === ba.eligible, `${lens} ${people[i].id}/${people[j].id} eligible asym`);
          assert(ab.band === ba.band, `${lens} band asym`);
          const dR = Math.abs(ab.rank - ba.rank);
          const dS = Math.abs(ab.sim - ba.sim);
          const dPW = Math.abs((ab.flags.pursueWithdraw ?? -1) - (ba.flags.pursueWithdraw ?? -1));
          const dHA = Math.abs((ab.flags.bothHighAgency ?? -1) - (ba.flags.bothHighAgency ?? -1));
          maxDiff = Math.max(maxDiff, dR, dS, dPW, dHA);
          assert(dR <= 1e-12 && dS <= 1e-12 && dPW <= 1e-12 && dHA <= 1e-12,
...
test('P3 monotone: raising weaker partner regulation never lowers romantic sim', () => {
  ...
  assert(firedAtLeastOnce, 'sweep never exercised the pursueWithdraw penalty path');
...
test('P4 gates: wantsKids mismatch fails; consent-off / no-photo excluded entirely', () => {
  ...
  assert((kids.reason ?? '').includes('wantsKids'), `reason was: ${kids.reason}`);
  assert(kids.rank === 0 && kids.drivers.length === 0, 'ineligible pair must carry no rank/drivers');

  // orientation + single gates
  const straightF = mk('of', { gender: 'F', interestedIn: ['M'] });
  const straightF2 = mk('of2', { gender: 'F', interestedIn: ['M'] });
  assert(!scorePair(straightF, straightF2, 'romantic').eligible, 'orientation gate failed to fire');
  const taken = mk('tk', { gender: 'M', single: false });
  assert(!scorePair(ky, taken, 'romantic').eligible, 'single gate failed to fire');

  // business gates
  assert(!scorePair(mk('r0', { risk: 0 }), mk('r2', { risk: 2 }), 'business').eligible, 'riskPosture gap 2 must fail');
  assert(!scorePair(mk('e0', { exit: 0 }), mk('e2', { exit: 2 }), 'business').eligible, 'exitHorizon gap 2 must fail');
  assert(!scorePair(mk('rl', { redlines: false }), mk('rr', {}), 'business').eligible, 'redlines gate failed to fire');
  ...
  assert(!rom.includes('noc'), 'no-consent person leaked into romantic ranking');
  assert(!rom.includes('nop'), 'no-photo person leaked into romantic ranking');
  const asSubject = rankRoom(room, 'noc', 'romantic');
  assert(asSubject.length === 0, 'no-consent subject must rank nobody (floor suppress)');
  const fri = rankRoom(room, 'sub', 'friendship').map((r) => r.id);
  assert(fri.includes('noc'), 'noc consented to friendship — must appear there');
  assert(!fri.includes('nop'), 'no-photo person leaked into friendship ranking');
  const excl = excludedFromRoom(room, 'sub', 'romantic');
  const exclNoc = excl.find((e) => e.id === 'noc');
  assert(exclNoc !== undefined && exclNoc.reason.includes('consent'), 'exclusion reason must name consent');

  return `wantsKids reason="${kids.reason}"; romantic ranking=[${rom.join(',')}] (noc,nop out); ` +
...
test('P5a business capacity gap: moving b away from a never increases rank/sim', () => {
  ...
  for (let i = 1; i < ranks.length; i++) {
    assert(ranks[i] < ranks[i - 1] - 1e-12, `rank did not strictly fall at gap ${i}: ${ranks[i - 1]} -> ${ranks[i]}`);
    assert(sims[i] < sims[i - 1] - 1e-12, `sim did not strictly fall at gap ${i}: ${sims[i - 1]} -> ${sims[i]}`);
  }
...
test('P5b friendship Distance max(): worsening b never changes score when a texts first', () => {
  ...
  const r1 = [1, 2, 3].map((db) => scorePair(a1, mk('b', { distanceBand: db, gender: 'M' }), 'friendship'));
  for (let i = 1; i < r1.length; i++) {
    assert(near(r1[i].rank, r1[0].rank, 1e-15), `friendship rank moved: ${r1[0].rank} -> ${r1[i].rank}`);
    assert(near(r1[i].sim, r1[0].sim, 1e-15), `friendship sim moved: ${r1[0].sim} -> ${r1[i].sim}`);
  }
  // a band 0 (texter, da = 1): b anywhere — "one texter is enough", still constant.
  const a0 = mk('a', { distanceBand: 0 });
  const r0 = [0, 1, 2, 3].map((db) => scorePair(a0, mk('b', { distanceBand: db, gender: 'M' }), 'friendship'));
  for (let i = 1; i < r0.length; i++) {
    assert(near(r0[i].rank, r0[0].rank, 1e-15), `texter-a rank moved: ${r0[0].rank} -> ${r0[i].rank}`);
  }
...
test('P5c kernels: money (rom) and chronotype (all lenses) — moving away never increases', () => {
  ...
  for (const bm of [0.5, 0.6, 0.7, 0.8, 0.9, 1.0]) {
    ...
    assert(s.rank <= prevR + 1e-15 && s.sim <= prevS + 1e-15, `romantic score rose as money diverged at ${bm}`);
    if (prevR !== Infinity) assert(s.rank < prevR - 1e-12, `gaussian kernel not strictly decreasing at ${bm}`);
    prevR = s.rank; prevS = s.sim;
    moneyRanks.push(s.rank);
  }
  parts.push(`rom rank vs money gap 0->0.5: ${moneyRanks[0].toFixed(4)} -> ${moneyRanks[5].toFixed(4)}`);
  ...
  for (const lens of LENSES) {
    ...
    for (const bc of [1, 2, 3]) {
      ...
      assert(s.rank <= pR + 1e-15 && s.sim <= pS + 1e-15, `${lens} score rose as chronotype diverged at ${bc}`);
      pR = s.rank; pS = s.sim;
      vals.push(s.rank);
    }
    parts.push(`${lens} rank vs chrono gap 0,1,2: ${vals.map((v) => v.toFixed(4)).join(',')}`);
  }
  return parts.join('  |  ');
});
...
test('P6 determinism: identical calls give byte-identical output', () => {
  const room = [
    mk('s1', { team: 't1', gender: 'F' }),
    mk('s2', { team: 't1', gender: 'M', agc: 0.9, reg: 0.2, distanceBand: 3, tags: ['music', 'gaming'], cohort: 3 }),
    mk('s3', { noLatents: true, distanceBand: 'missing', gender: 'NB', tags: [], cohort: 5 }),
    mk('s4', { gender: 'M', pol: 0.9, cap: 0, chrono: 3, wantsKids: false, acq: ['s1'] }),
...
test('P7 degraded: missing latents impute prior 0.5/se 0.6; ranked; NO weight renorm', () => {
  ...
  const explicit = mk('gap', { reg: 0.5, pol: 0.5, rel: 0.5, agc: 0.5, se: 0.6, gender: 'NB' });
  ...
  const stripFlag = (s: ReturnType<typeof scorePair>): string =>
    JSON.stringify({ ...s, flags: { ...s.flags, bothHighAgency: undefined } });
  for (const lens of LENSES) {
    const a = scorePair(gap, partner, lens);
    const b = scorePair(explicit, partner, lens);
    assert(stripFlag(a) === stripFlag(b), `${lens}: missing-latent person scores differ from explicit prior person`);
    if (lens !== 'friendship') {
      assert(a.flags.bothHighAgency === undefined, `${lens}: imputed prior surfaced bothHighAgency`);
      assert(b.flags.bothHighAgency !== undefined, `${lens}: measured posterior must display the probability`);
    }
  }
  ...
  for (const lens of LENSES) {
    ...
    assert(ids.includes('gap'), `${lens}: degraded person fell out of the ranking`);
  }
  ...
  for (const lens of LENSES) {
    ...
    assert(near(d, expected[lens], 1e-9), `${lens}: Δrank=${d}, expected ${expected[lens]} (weights renormalized?)`);
...
test('P8 business Agency default off: off-vector sums to 1; overlay restores .05/.10 and changes scores', () => {
  ...
  assert(off.rank.agency === 0 && off.sim.agency === 0, 'default business agency weight must be 0');
  assert(on.rank.agency === 0.05 && on.sim.agency === 0.10, 'overlay must restore .05/.10');
  const sumOffRank = TERMS.reduce((s, t) => s + off.rank[t], 0);
  const sumOffSim = TERMS.reduce((s, t) => s + off.sim[t], 0);
  assert(near(sumOffRank, 1, 1e-9) && near(sumOffSim, 1, 1e-9), `off sums: ${sumOffRank}, ${sumOffSim}`);
  ...
  assert(Math.abs(sOff.rank - sOn.rank) > 1e-9, 'overlay had no effect on rank');
  ...
  assert(sOff.flags.bothHighAgency !== undefined && near(sOff.flags.bothHighAgency, expectedBoth, 1e-9),
    `bothHighAgency=${sOff.flags.bothHighAgency}, expected ${expectedBoth}`);
  assert(sOn.flags.bothHighAgency !== undefined, 'flag must fire under overlay too');
  return `off sums rank=${sumOffRank.toFixed(12)} sim=${sumOffSim.toFixed(12)}; ` +
...
test('P9 pursueWithdraw: P(bottom-band reg) x [partner distanceBand==3] > 0.60 fires (symmetric)', () => {
  ...
  assert(s1.flags.pursueWithdraw !== undefined && near(s1.flags.pursueWithdraw, expectedP, 1e-9),
    `flag=${s1.flags.pursueWithdraw}, expected ${expectedP}`);
  const s2 = scorePair(far, anx, 'romantic'); // symmetric direction
  assert(s2.flags.pursueWithdraw !== undefined && near(s2.flags.pursueWithdraw as number, expectedP, 1e-12), 'flag not symmetric');
  assert(scorePair(calm, far, 'romantic').flags.pursueWithdraw === undefined, 'fired below threshold');
  assert(scorePair(anx, nearBy, 'romantic').flags.pursueWithdraw === undefined, 'fired without distanceBand==3');
  ...
  return `flag=${(s1.flags.pursueWithdraw as number).toFixed(4)} (=normCdf(.40;.20,.45)=${expectedP.toFixed(4)}) > 0.60; ` +
...
test('P9b numeric sanity: erf, normCdf, frozen band cutoffs', () => {
  ...
  assert(near(erf(-1), -0.8427007929, 1.5e-7), `erf(-1)=${erf(-1)}`);
  assert(near(normCdf(0.5, 0.5, 0.45), 0.5, 1.5e-7), 'normCdf at mean != 0.5');
  assert(bandOf(0.399999) === 'low' && bandOf(0.4) === 'mid' && bandOf(0.599999) === 'mid' && bandOf(0.6) === 'high',
