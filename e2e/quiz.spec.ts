...
test.describe("quiz", () => {
  ...
  // (room + participant + hookai_session cookie, 15 generated_blocks rows
  // seeded through GeneratedBlockRepository.saveBatch).
  test.skip('AC-1 · "Empezar" opens the participant\'s seeded block 1 with its scenario, its four option texts as card labels, "1/15", a disabled "Siguiente", no "Atrás" and all four cards inside 390×844', async () => {});
  ...
  test.skip('AC-2 · a tap marks "Más yo", a re-tap clears it, a second card takes "Menos yo", and "Siguiente" writes one row at position 1 with most c, least d and shown_order = shownOrderFor(participantId, 1), then shows the participant\'s block 2 as "2/15"', async () => {});
  ...
  // Blocked on: batch-beat.tsx ("Tanda N de 3", "Seguir" → ?start=1; the
  // opened batch is already in generated_blocks when it renders) and
  // e2e/helpers/quiz-participant.ts seeding positions 1-4 and reading
  // generated_blocks back.
  test.skip('AC-4 · block 5 opens "Tanda 2 de 3" with "Seguir" and no option text, "Seguir" shows the participant\'s block 6 as "6/15", no beat appears between 6 and 10, block 10 opens "Tanda 3 de 3" onto block 11, and the participant keeps ten rows and exactly 15 generated_blocks rows', async () => {});

  // TODO: un-skip when back and ?block exist.
  // Blocked on: "Atrás" rendering the previous block pre-marked, ?block=N
  // clamped to the first unanswered position (the block for that position
  // read through ensureQuizBatch), answerBlockAction redirecting a re-answer
  // to /quiz?start=1 (no beat, no forward walk) and
  // e2e/helpers/quiz-participant.ts seeding positions 1-7.
  test.skip('AC-5 · "Atrás" and ?block=3 re-answer an earlier block of the participant\'s form in place and return straight to block 8; ?block=12 is clamped to block 8', async () => {});
  ...
  test.skip('AC-6 · the participant\'s block 15 is shown directly with "15/15" and "Terminar", which sets quiz_completed_at, lands on /results "Listo" and sends /quiz there afterwards', async () => {});
});

test.describe("safety invariants", () => {
  // Runs today (kind: safety). Exactly one option per block is reversed-keyed,
  // each belongs to a pillar and each stored block names its focus pillar;
  // nothing rendered -- DOM text, alt text, the RSC payload serialized into
  // the HTML -- may say which (PILLARS.md §8 rule 1, AUDIT.md F1,
  // CLAUDE_DESIGN_QUIZ_BLOCK.md §3). Today /quiz does not exist, so those
  // pages are 404s and the check is vacuous; when the quiz lands, seed a
  // participant with no responses and 15 generated_blocks rows through
  ...
  // screen additionally expect that participant's block's four option texts
  // to be visible as the card labels -- keeping the assertions below on every
